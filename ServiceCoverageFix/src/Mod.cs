    ...
    public sealed class Mod : IMod
    {
        ...
        public void OnLoad(UpdateSystem updateSystem)
        {
            try
            {
                ...
                ApplyCoverageScheduleBridge.InitializeInputDiagnostics();
                ...
                updateSystem.UpdateAt<DiagnosticControlSystem>(SystemUpdatePhase.GameSimulation);
                DiagnosticControlSystem.SetRuntimeControlsEnabled(true);
                ...
                Log.Info(
                    $"Enabled ServiceCoverageFix {modVersion} radix diagnostic-control build " +
                    $"with packaged Harmony {harmonyVersion}. " +
                    "Ctrl+Shift+F9 toggles coverage logging; hold Ctrl+Shift+F10 for 3 seconds " +
                    "to delete all parks, park-maintenance facilities, and active park-maintenance vehicles.");
            }
            catch (Exception exception)
            {
                DiagnosticControlSystem.SetRuntimeControlsEnabled(false);
                try
                {
        ...
        {
            DiagnosticControlSystem.SetRuntimeControlsEnabled(false);
            ApplyCoverageScheduleBridge.ShutdownInputDiagnostics();
            _harmony?.UnpatchAll(HarmonyId);
            _harmony = null;
        }
    }
