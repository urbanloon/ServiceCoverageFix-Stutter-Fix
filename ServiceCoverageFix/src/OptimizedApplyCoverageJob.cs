...
namespace ServiceCoverageFix
{
    ...
    internal unsafe struct OptimizedApplyCoverageJob : IJob
    {
        ...
        [NativeDisableContainerSafetyRestriction]
        internal NativeArray<int> InputDiagnostics;

        [BurstCompile]
        public void Execute()
        {
            // The Prepare dependency populates both lists after this job is
            // scheduled. Capture their lengths here, where the dependency has
            // completed, rather than from the managed scheduling bridge.
            if (InputDiagnostics.IsCreated)
            {
                InputDiagnostics[0] = BuildingData.Length;
                InputDiagnostics[1] = Elements.Length;
                InputDiagnostics[2] = 0;
            }
            ...
            // ProcessCoverageJob NativeSorts every provider range immediately
            // before this job. This project is hash-locked to that exact game
            // implementation, and the v0.1.4 traces proved the radix invariant
            // holds in both pathological saves. Do not redundantly scan all E
            // elements a second time here: Providence Bay's Park pass alone has
            // 1,467,894 elements (about 47 MB at the native 32-byte stride).
            int processedElements = ExecuteRadixMerge(
                records,
                elements,
                BuildingData.Length);
            ...
            if (InputDiagnostics.IsCreated)
            {
                InputDiagnostics[2] = processedElements;
            }
        }
        ...
        private static int ExecuteRadixMerge(
            RawBuildingData* records,
            RawCoverageElement* elements,
            int recordCount)
        {
